...
- 日常扫描：优先做最近窗口的增量刷新，只覆盖最近重叠日期
- 周期校验：隔几天自动做一次全量刷新，用于修正前复权历史和防止老数据漂移

这样既能提速，也能避免每天重写整段历史数据。
...
- 回测完成后会自动发送一条飞书摘要，直接比较最近 `1 个月` 与 `6 个月` 的胜率、斜率和回撤变化
## 七、回测使用方式
...
- `backtest_summary.json`
  汇总统计，包括样本数、胜率、平均收益、累计收益、斜率、最大回撤、以及整体/正式命中/候选观察分组表现
